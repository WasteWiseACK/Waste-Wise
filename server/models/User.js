...
class User {
  ...
  constructor({ id, username, password_hash, borough, bio, other_form_of_contact, profile_pic }) {
    this.id = id;
  ...
  static async update(id, updates) {
    const { username, borough, bio, other_form_of_contact, profile_pic } = updates;
    console.log('Updating user with ID:', id, 'Username:', username, 'Bio:', bio, 'Borough:', borough, 'profile_pic:', profile_pic);
    const query = `
      UPDATE users
      SET 
      ...
      RETURNING *
    `
    const result = await knex.raw(query, [username || null, borough || null, bio || null, other_form_of_contact || null, profile_pic || null, id])
